...
class _AttendancePageState extends State<AttendancePage> {
  ...
  String shift='morning',message=''; int lateGrace=15,earlyGrace=5;
  ...
  Map<String,int> analyze(DateTime d){if(d.weekday==DateTime.friday)return{'worked':0,'late':0,'early':0,'status':0};final i=ins[key(d)],o=outs[key(d)];if(i==null&&o==null)return{'worked':0,'late':0,'early':0,'status':1};if(i==null||o==null)return{'worked':0,'late':0,'early':0,'status':2};final m=worked(i,o);if(shift=='flexible')return{'worked':m,'late':0,'early':0,'status':3};final si=shift=='morning'?360:840,so=shift=='morning'?840:1320;var ai=tmin(i),ao=tmin(o);if(ao<=ai)ao+=1440;final rawLate=(ai-si).clamp(0,9999).toInt(),rawEarly=(so-ao).clamp(0,9999).toInt();return{'worked':m,'late':rawLate>lateGrace?rawLate:0,'early':rawEarly>earlyGrace?rawEarly:0,'status':3};}
