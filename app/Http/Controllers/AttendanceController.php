...
class AttendanceController extends Controller {
 ...
 public function save(Request $request): RedirectResponse {
  $v=$request->validate(['employee_id'=>['required','exists:employees,id'],'start_date'=>['required','date'],'end_date'=>['required','date','after_or_equal:start_date'],'shift_type'=>['required','in:morning,evening,flexible'],'rows'=>['nullable','array'],'rows.*.check_in'=>['nullable','date_format:H:i'],'rows.*.check_out'=>['nullable','date_format:H:i']]);
  $employee=Employee::findOrFail($v['employee_id']); $employee->update(['shift_type'=>$v['shift_type']]);
  ...
  return redirect()->route('attendance.sheet',['employee_id'=>$v['employee_id'],'start_date'=>$v['start_date'],'end_date'=>$v['end_date']])->with('success','تم حفظ الدوام وإعادة الحساب وفق سياسة السماح العامة.');
 }
 ...
 private function analyze(Carbon $date, ?Attendance $a, Employee $employee): array {
  ...
  return ['minutes'=>$minutes,'status'=>'present','late'=>$rawLate>$lateGrace?$rawLate:0,'early'=>$rawEarly>$earlyGrace?$rawEarly:0];
 }
